...
MAX6675 and VDRIVE2 pins are defined below (they are the digital pins).
...
// Global Variables
byte second, myminute, hour, dayOfWeek, dayOfMonth, month, year;
...
void getDateDs1307(char* filename)
{
//  byte isecond, iminute, ihour, iday, idayOfWeek, idayOfMonth, imonth, iyear;
...
void setup()
{
  ...
  /*Serial.println("Sensors on, delaying.");

  digitalWrite(SENSORS_ENABLE_PIN,0); // enable sensors to set time
  delay(5000);*/
...
//  digitalWrite(SENSORS_ENABLE_PIN,1); // disable sensors
//  delay(1000);
}
...
/*  Serial.println("Recording temperature...");
  temperature = temp_sensor.read_temp(5);
...
/*void make_filename(char* filename, time_t log_time)
{
  print_two_digits(filename,day(log_time),0);
  print_two_digits(filename,hour(log_time),2);
  print_two_digits(filename,minute(log_time),4);
  print_two_digits(filename,second(log_time),6);
  filename[8] = '.'; filename[9] = 't'; filename[10] = 'x'; filename[11] = 't';
  filename[12] = 0;
}

void print_two_digits(char* s, int what, int where)
{
  s[where] = '0' + what/10;
  s[where+1] = '0' + what%10;
}*/
...
void record()
{
  ...
  while( num_d < NUM_SAMPLES ) {
/*    Serial.print("num_d=");
    Serial.println(num_d);*/
    delay(100);
  }
